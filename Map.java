...
public class Map {
	...
	public Map(int numVertices) {
		...
		// where the LinkedList has a head (containing the Intersection) which point to
		// an Edge
		...
		// set the minimum and maximum latitudes and longitudes to appropriate starting
		// integer values
	...
	public static String formPath(String endID) {
		...
		// adds the Vertex's path to the path[] until we reach the start Vertex (which
		// has no path)
		...
		// now working backwards from path[], create a string of the path from the start
	...
	// method to determine the total distance required to travel between the
	// intersections
	...
	public void createSet() {
		...
		// instantiate the HashMap that maps IntersectionID to HashSet of vertices
		// connected to that intersection
	...
	// METHOD THAT DETERMINES THE ROADS THAT MAKE UP THE MINIMUM WEIGHT SPANNING
	// TREE
	...
	// METHOD THAT DETERMINES THE PATHS FOLLOWED TO GET FROM START TO FINISH IN THE
	// SHORTEST DISTANCE
	...
	public void dijkstra(String intersectionID) {
		...
		// for speed -> set the number of unknown vertices to the total number of
		// vertices and decrement it when a vertex is set to known
		int numUnknownVertices = intersectionMap.size();

		while (numUnknownVertices > 0) {

			// smallestUnknownVertex() returns the unknown vertex with the smallest distance
			// from the heap of intersections
			...
			while (currentRoad != null) {
				...
				// we want the intersection that is NOT the same as the one we are currently
				// visiting
	...
	// method that calculates the distance between two pairs of longitude and
	// latitude
	// adapted from the Haversine Formula found on
